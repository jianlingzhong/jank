...
namespace jank::read::lex
{
  ...
  static std::ostream &operator<<(std::ostream &os, std::variant<Ts...> const &v)
  {
    std::visit(
      [&](auto &&arg) {
        ...
        else if constexpr(std::is_same_v < T, runtime::obj::native_bigint)
        {
          os << arg.str();
        }
        else
  ...
  token::token(movable_position const &s,
               movable_position const &e,
               token_kind const k,
               runtime::obj::native_bigint const d)
    : start{ s }
    , end{ e }
    , kind{ k }
    , data{ d }
...
#ifdef JANK_TEST token::token(
    size_t const offset,
    size_t const width,
    token_kind const k) : start{ offset, 1, offset + 1 },
                                 end{ offset + width, 1, offset + width + 1 }, kind{ k }
    {
    }
        ...
        case '0' ... '9':
          {
            ...
            while(true)
            {
              ...
              if(auto const [c, len](result.unwrap_or(codepoint{ ' ', 1 })); c == '.')
              {
                ...
                {
                  ++pos;
                  return error::lex_invalid_number("An arbitrary radix number can only be an "
                                                   "integer, so it may not contain a '.'.",
                                                   { token_start, pos },
              ...
              else if(c == '/')
              {
                ...
                if(denominator.is_ok())
                {
                  ...
                  {
                    return error::lex_invalid_ratio("A ratio denominator must be an integer.",
                                                    {
                                                      token_start,
                                                      pos
                    },
                                                    error::note{ "Denominator is here.",
                                                                 { denominator.expect_ok().start,
                                                                   denominator.expect_ok().end } });
                  }
           ...
           * followed by a number. Otherwise, it's a symbol. */
            ...
            if(file[token_start] != '-' || (pos - token_start) >= 1)
            {
              ...
              if(errno == ERANGE)
              {
                ...
                std::string num_str(file.data() + number_start, pos - number_start);
                runtime::obj::native_bigint parsed_bigint;

                try
                {
                  if(radix == 16)
                  {
                    parsed_bigint.assign("0x" + num_str);
                  }
                  else if(radix == 8)
                  {
                    parsed_bigint.assign("0" + num_str);
                  }
                  else if(radix == 10)
                  {
                    parsed_bigint.assign(num_str);
                  }
                  else
                  {
                    // Radix other than 8, 10, 16 is not directly supported by cpp_int string parsing easily
                    return error::lex_invalid_number(
                      util::format("Bigint parsing for base {} is not implemented.", radix),
                      { token_start, pos });
                  }

                  if(found_beginning_negative)
                  {
                    parsed_bigint = -parsed_bigint;
                  }
                }
                catch(std::exception const &e)
                {
                  return error::lex_invalid_number(
                    util::format("Failed to parse bigint: {}", e.what()),
                    { token_start, pos });
                }
                return ok(token{ token_start, pos, token_kind::bigint, parsed_bigint });
              }
              ...
              case '?':
                {
                  ...
                  if(character == '@')
                  {
                    ++pos;
                    return ok(
                      token{ token_start, pos, token_kind::reader_macro_conditional_splice });
                  }
                  else
                  {
